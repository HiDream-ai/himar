...
from util.loader import SmallCachedFolder

from models.vae import AutoencoderKL
from models import himar
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='himar_large', type=str, metavar='MODEL',
    ...
    parser.add_argument('--re_cfg', default=1.0, type=float, help="classifier-free guidance")
    ...
    parser.add_argument('--two_diffloss', action='store_true')
    parser.add_argument('--global_dm', action='store_true')
    ...
    parser.add_argument('--step_warmup', type=int, default=100, metavar='N',
                        )
    ...
    parser.add_argument('--cond_scale', type=int, default=4)
    parser.add_argument('--cond_dim', type=int, default=16)
    ...
    parser.add_argument('--gdm_d', type=int, default=3)
    parser.add_argument('--gdm_w', type=int, default=768)
...
def main(args):
    ...
    if not args.evaluate:
        ...
        if args.use_cached:
            dataset_train = SmallCachedFolder(args.cached_path)
        else:
    ...
    # define the vae and himar model
    vae = AutoencoderKL(embed_dim=args.vae_embed_dim, ch_mult=(1, 1, 2, 2, 4), ckpt_path=args.vae_path).cuda().eval()
    for param in vae.parameters():
        param.requires_grad = False
    
    model = himar.__dict__[args.model](
        ...
        diff_seqlen=args.diff_seqlen,
        step_warmup=args.step_warmup,
        step_stage2_rate=args.step_stage2_rate,
        cond_scale=args.cond_scale,
        cond_dim=args.cond_dim,
        two_diffloss=args.two_diffloss,
        seq_len=args.seq_len,
        global_dm=args.global_dm,
        gdm_w=args.gdm_w,
        gdm_d=args.gdm_d,
        head=args.head,
        ratio=args.ratio,
    ...
    if args.load_epoch == -1:
        ...
        load_path = f"checkpoint-{args.load_epoch}.pth"
    if args.resume and os.path.exists(os.path.join(args.resume, load_path)):
        checkpoint = torch.load(os.path.join(args.resume, load_path), map_location='cpu')
        if args.evaluate:
            ...
            if 'optimizer' in checkpoint and 'epoch' in checkpoint:
                ...
                model_without_ddp.epoch = checkpoint['epoch']
        ...
        if args.online_eval and (epoch % args.eval_freq == 0 or epoch + 1 == args.epochs) and (epoch > 0):
            ...
            if not (args.cfg == 1.0 or args.cfg == 0.0):
                evaluate(model_without_ddp, vae, ema_params, args, epoch, batch_size=args.eval_bsz,
                         log_writer=log_writer, cfg=args.cfg, use_ema=True,vq_model=vq_model,model_test=model_test,ema_test_params=ema_test_params)
            torch.cuda.empty_cache()
